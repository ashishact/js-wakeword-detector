<script lang="ts">
    ...
    class CircularAudioWave {
        ...
        offlineSource: AudioBufferSourceNode;
        ...
        _init() {
            ...
            this._debouncedDraw = this._debounce(this._drawAnimation.bind(this), 25, false);
        }
        presetOption() {
            if (this.opts.mode !== 'sunburst') {
        ...
        toggle() {
            if(this.playing){
                this.sourceNode.playbackRate.value = 0;
                this.playing = false;
            }
            else{
    ...
    let wave:CircularAudioWave|null = null;
    let begin = ()=>{
        // wave = new CircularAudioWave(document.getElementById('chart-container'), {mode: 'sunburst'});
        wave = new CircularAudioWave(document.getElementById('chart-container'));
        wave.loadAudio('assets/ballade_pour_adeline.mp3');
        // wave.loadAudio('assets/thisisatest.mp3');
